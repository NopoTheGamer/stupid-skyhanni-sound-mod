...
echo "Setting project name to $mod_name"
echo "Setting package dir to $package_dir"
echo "Setting mod id to $modid"
(
  set -x
  find "$base"/src/main -type f -exec sed -i s/examplemod/"$modid"/g\;s/com.example/"$package_name"/g\;s/ExampleMod/"$mod_name"/g {} +
  sed -i s/com.example/"$package_name"/g\;s/examplemod/"$modid"/g\;s/ExampleMod/"$mod_name"/g "$base"/gradle.properties
  sed -i s/examplemod/"$mod_name"/g "$base"/settings.gradle.kts
